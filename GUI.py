...
Company: Critical Innovations
Project: AutoFOAM
Written by: Grant Ingamells
Date: 2023/10/04
Modified: 2023/10/24

Target hardware: Raspberry Pi Attached to Arduino

VERSIONS:
    0.10 - GI - Initial release
    0.11 - TM - Updates to pause process
'''
...
class App(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        self.pause_event = threading.Event()  # Create a threading event for pause/resume
        self.running_thread = None  # Store the thread that runs CylDrive
    ...
    def pause_button_callback(self):
        if self.running_thread and self.running_thread.is_alive():
            self.pause_event.set()  # Set the pause event to pause the thread
            print("Pausing...")
        else:
            print("No process running to pause.")
            
    def resume_button_callback(self):
        if self.running_thread and not self.running_thread.is_alive():
            self.running_thread = None  # Clear the old thread reference
        if self.running_thread is None:
            self.pause_event.clear()  # Clear the pause event to resume the thread
            self.running_thread = threading.Thread(target=self.CylDrive)
    ...
    def CylDrive(self, cylStatusin, mfcTrigger):
        ...
        while self.running == True:
         if self.pause_event.is_set():
                time.sleep(0.1)  # Sleep briefly to allow pausing to take effect
                continue
